...
export class AuthService {
    ...
    private async getOculusUser(accessToken: string) {
        const resp = await fetch(`https://graph.oculus.com/me?access_token=${accessToken}&fields=id,alias`, {
            method: 'GET',
            headers: {
    ...
    async authenticateOculus(signInOculusDto: AuthenticateOculusDto) {
        ...
        let chatwoUser = await this.userRepository.findOne({ where: { oculusId: user.id } });
        if (!chatwoUser) {
        ...
        return this.nakamaService.authenticate(chatwoUser.nakamaId, signInOculusDto.username ?? user.alias);
    }

    async oculusExists(id: string): Promise<{ exists: boolean }> {
        const chatwoUser = await this.userRepository.findOne({ where: { oculusId: id } });
