...
export class HiltService {
  ...
  async create(hiltData: CreateHiltDto): Promise<ChatwoHilt> {
    const user = await this.userRepository.findOne({
      where: { nakamaId: hiltData.ownerNakamaId },
    });
    if (!user) {
  ...
  async update(id: number, hiltData: UpdateHiltDto): Promise<ChatwoHilt> {
    ...
    try {
      const hilt = await this.hiltRepository.findOne({
        where: { id },
        relations: { blade: true, owner: true },
      });
      if (!hilt) {
        throw new NotFoundException('Hilt not found');
      }
      ...
      if (
        hilt.owner.nakamaId !== hiltData.ownerNakamaId &&
        hilt.state !== HiltEquipState.OUT_OF_CONTROL
      ) {
      ...
      if (
        hiltData.bladeKey !== undefined &&
        hilt.blade?.bladeKey !== hiltData.bladeKey
      ) {
        if (hilt.blade) {
  ...
  async findAllAdmin(): Promise<ChatwoHilt[]> {
    return this.hiltRepository.find({
      relations: { blade: true, owner: true },
    });
  }
}
